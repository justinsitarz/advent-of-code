...
def populate_coordinates(input_list):
	...
	for ind, val in enumerate(input_list):
		...
		if val[0] == "U":
			for x in range(1,value):
				new_coordinates = (prior_coordinates[0], prior_coordinates[1] + x)
				coordinates.append(new_coordinates)
		if val[0] == "D":
			for x in range(1,value):
				new_coordinates = (prior_coordinates[0], prior_coordinates[1] - x)
...
def check_for_crossings(coords_1, coords_2):
	crossing_list = []
	for c in coords_1:
		print(c)
		if c in coords_2:
			crossing_list.append(c)
	return crossing_list


def check_closest_crossing(crossing_list):
	totals_set = {0}
	for c in crossing_list:
		new_value = abs(c[0]) + abs(c[1])
		if new_value == 860:
			print(c)
		totals_set.add(abs(c[0]) + abs(c[1]))
...
wire1_coordinates = set(populate_coordinates(wire1_input))

wire2_coordinates = set(populate_coordinates(wire2_input))

crossing_list = check_for_crossings(wire1_coordinates, wire2_coordinates)

closest = check_closest_crossing(crossing_list)
